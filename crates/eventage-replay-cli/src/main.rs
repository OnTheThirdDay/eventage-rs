...
//!
//! ```bash
//! cargo run -p eventage-replay-cli -- path/to/events.jsonl [--port <port>]
//! ```
//!
    ...
    if args.len() < 2 || args[1] == "--help" || args[1] == "-h" {
        eprintln!(
            "Usage: eventage-replay <events.jsonl> [--port <port>]\n\
             ...
             The JSONL file is produced by eventage::observability::JsonlExporter.\n\
             \n\
             For live replay during execution, use eventage::replay::LiveReplayServer."
        );
        std::process::exit(1);
    }
