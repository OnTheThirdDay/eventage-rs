...
//! use eventage_sqlite::SqliteEventStore;
//!
...
//! ```no_run
//! use eventage_core::EventBus;
//! use eventage_observability::BusObserver;
...
//! ```
//!
//! # Enabling bundled SQLite
//!
//! To avoid a system SQLite dependency, enable the `bundled` feature:
//!
//! ```toml
//! [dependencies]
//! eventage-sqlite = { version = "0.1", features = ["bundled"] }
//! ```

use async_trait::async_trait;
use eventage_core::Event;
use eventage_observability::ObservabilityExporter;
...
///
/// Pair with [`eventage_observability::BusObserver`] for zero-configuration persistent event logging:
///
/// ```no_run
/// use eventage_core::EventBus;
/// use eventage_observability::BusObserver;
/// use eventage_sqlite::SqliteExporter;
///
/// # async fn example() -> Result<(), Box<dyn std::error::Error>> {
/// let bus = EventBus::new();
/// let exporter = SqliteExporter::new("./agent.db").await?;
...
impl ObservabilityExporter for SqliteExporter {
    async fn export(&self, event: &Event) -> Result<(), eventage_observability::ObsError> {
        self.store
            .append(event)
            .await
            .map_err(|e| eventage_observability::ObsError::Other(e.to_string()))
    }

    async fn flush(&self) -> Result<(), eventage_observability::ObsError> {
