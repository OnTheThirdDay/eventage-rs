use crate::context::{AssemblyContext, ContextAssembler};
use crate::error::AgentError;
use crate::hook::CycleHook;
use crate::strategy::{AgentContext, ExecutionStrategy};
use crate::tool::{ToolRegistry, ToolSelector};
use eventage_core::{kinds, meta_keys, EventBus};
...
impl Agent {
    ...
    pub async fn cycle(&self) -> Result<(), AgentError> {
        ...
        let cycle_start = eventage_core::Event::new(kinds::AGENT_CYCLE_START, json!({}))
        ...
        let cycle_end = eventage_core::Event::new(kinds::AGENT_CYCLE_END, json!({}))
            .with_meta(meta_keys::AGENT_ID, json!(self.agent_id))
            .with_meta(meta_keys::TRACE_ID, json!(&trace_id))
