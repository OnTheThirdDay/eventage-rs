//! WASM-based sandboxed execution via [wasmtime](https://wasmtime.dev/) + WASI.
//!
//! `WasmExecutor` runs `.wasm` binaries in a fully isolated WebAssembly
//! environment. The module cannot access the network, host resources, or
//! the host filesystem beyond explicitly whitelisted directories.
//!
//! # Requirements
//! `SandboxRequest.program` must point to a valid `.wasm` file.
//!
//! # Execution Guarantees
//! - A fresh instance is created for each `execute` call (no state leaks).
//! - Stdin/stdout/stderr are properly piped.
//! - Only paths in `readable_paths` and `writable_paths` are mounted.
//! - `working_dir` is mounted as `/sandbox/work`.
//!
//! # Example
//!
//! ```no_run
//! use eventage_sandbox::{WasmExecutor, SandboxExecutor, SandboxRequest};
//! use std::collections::HashMap;
//! use std::path::PathBuf;
//!
//! # async fn example() -> Result<(), Box<dyn std::error::Error>> {
//! let executor = WasmExecutor::new();
//! let output = executor.execute(SandboxRequest {
//!     program: "/path/to/module.wasm".into(),
//!     args: vec!["--flag".into()],
//!     env: HashMap::new(),
...
impl WasmExecutor {
    ...
    pub fn new() -> Self {
        ...
        config.async_support(false); // We wrap in spawn_blocking; sync is fine.
...
fn run_wasm(engine: Engine, req: SandboxRequest) -> Result<SandboxOutput, SandboxError> {
    ...
    // Command-line args (argv[0] = program name by convention)
    builder.arg(&req.program);
    for arg in &req.args {
        builder.arg(arg);
    }

    // Preopened directories — writable first, then read-only
    ...
    // Enforce timeout via fuel (approximate: 1 fuel ~ 1 WASM instruction)
    let fuel_limit = (req.timeout_ms as u64).saturating_mul(1_000_000);
    store.set_fuel(fuel_limit).ok(); // ignore error if fuel not configured
    ...
    // Call the WASI entry point: `_start` (equivalent to `main`).
    let start = instance
    ...
    let timed_out = match start.call(&mut store, ()) {
        Ok(()) => false,
        Err(e) => {
            // WASM can exit via wasi proc_exit(0) which is normal success.
