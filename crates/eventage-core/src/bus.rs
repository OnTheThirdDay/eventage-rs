...
mod tests {
    ...
    async fn rollback_seals_rejected_branch() {
        ...
        // Publish a base event, then checkpoint, then a failed event.
        ...
        // Active path is now truncated to just the user.message.
        assert_eq!(bus.log_len().await, 1);
        let log = bus.log().await;
        assert_eq!(log[0].kind, kinds::USER_MESSAGE);

        // The rejected branch contains checkpoint + assistant.message.
    ...
    async fn new_events_after_rollback_continue_from_anchor() {
        ...
        // Publish a fresh response on the new branch.
        bus.publish(Event::new(
            kinds::ASSISTANT_MESSAGE,
            json!({"content": "retry"}),
        ))
        ...
        // Parent of the retry is the user.message, not the checkpoint.
    ...
    async fn bounded_branches_evict_oldest() {
        ...
        // Helper: publish a user message + checkpoint + bad assistant + rollback.
        ...
        // Seed the bus with an initial event so rollbacks have an anchor.
        bus.publish(Event::new(
            kinds::USER_MESSAGE,
    ...
    async fn pruned_event_is_broadcast() {
        ...
        // First branch — fits within limit.
        ...
        // Second branch — triggers eviction of the first.
        ...
        // Drain the receiver and look for system.pruned.
        let mut found_pruned = false;
        // We know a bounded number of events were published; drain them all.
        while let Ok(event) =
    ...
    async fn bounded_subscriber_drops_on_overflow() {
        // With capacity=2 and 5 publishes, the subscriber should receive at most
        // 2 events (the queue fills before it is drained).
        let bus = EventBus::with_config(BusConfig {
        ...
        // Publish 5 events synchronously before the receiver drains.
        for i in 0..5usize {
        ...
        // Drain with a short timeout; expect exactly 2 events (queue capacity).
        ...
        // The in-memory log is unaffected — all 5 events are retrievable.
        assert_eq!(
            bus.log_len().await,
            5,
    ...
    async fn no_capacity_limit_high_volume() {
        // Publish many events rapidly; with broadcast this would lag slow
        // subscribers, with unbounded mpsc every event is delivered.
        let bus = EventBus::new();
        let mut rx = bus.subscribe();
        ...
        let mut count = 0usize;
        while count < n {
            // Use a timeout so the test doesn't hang if something breaks.
